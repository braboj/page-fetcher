...
Three-tier fetching strategy (auto-escalates on failure):
  1. urllib (plain HTTP) — fastest (~1s), works for most static pages
  2. Playwright (headless Chromium) — for JS-rendered pages (~5-9s)
  3. SeleniumBase UC mode — for bot-protected sites (~18-24s)

Auto mode tries urllib first. If bot protection is detected (captcha,
403 page), skips Playwright and goes straight to UC. If urllib fails
for other reasons (404, timeout), tries Playwright first then UC.

Batch mode keeps one Chrome session open for all URLs (~12s launch once,
then ~6s per page instead of ~18s per page).
...
Usage:
    ...
    py tools/fetch-page.py --batch urls.txt --uc          # batch with UC
...
def _fetch_single(
    ...
    force_js: bool, force_uc: bool, no_cache: bool,
    sb_session=None,
) -> str:
    ...
    if force_uc:
        ...
        if result and result != _BOT_BLOCKED:
            ...
            print("[auto] Skipping Playwright (bot protection), escalating to UC...", file=sys.stderr)
            if sb_session:
                content = _fetch_uc_with_session(sb_session, url, raw_html, wait_ms) or ""
            else:
                content = fetch_uc(url, raw_html, wait_ms) or ""
            ...
            content = fetch_playwright(url, raw_html, wait_ms)
            if not content:
                print("[auto] Escalating to SeleniumBase UC...", file=sys.stderr)
                if sb_session:
                    content = _fetch_uc_with_session(sb_session, url, raw_html, wait_ms) or ""
                else:
                    content = fetch_uc(url, raw_html, wait_ms) or ""
...
def _run_batch(
    ...
    Chrome launches once (~12s) and stays open for all pages. Each
    subsequent page only costs navigation + wait (~6s).
    ...
    # Determine if we need a persistent UC session
    # Check first URL with urllib to see if bot protection is present
    needs_uc = force_uc
    if not needs_uc and not force_js and urls:
        result = fetch_urllib(urls[0], raw_html)
        if result == _BOT_BLOCKED:
            needs_uc = True
        elif result:
            # First URL worked with urllib — might not need UC at all
            # We'll handle per-URL in the loop
            pass
    ...
    try:
        ...
        for i, url in enumerate(urls):
            ...
            content = _fetch_single(
                url, raw_html, wait_ms, force_js, force_uc, no_cache,
...
def main() -> None:
    ...
    flags = {"--html", "--no-cache", "--js", "--uc", "--wait", "--batch", "--output-dir"}
    skip_next = False
    urls = []
    ...
    if len(urls) == 1 and not output_dir:
        ...
        content = _fetch_single(urls[0], raw_html, wait_ms, force_js, force_uc, no_cache)
        sys.stdout.buffer.write(content.encode("utf-8", errors="replace"))
        sys.stdout.buffer.write(b"\n")
    else:
        # Batch mode — persistent browser session
        _run_batch(urls, raw_html, wait_ms, force_js, force_uc, no_cache, output_dir)
